...
def f1_pre_rec_tlink(labels, preds):
    ...
    return {
        "precision": prec.compute(predictions=preds, references=labels, average='micro'),
        "recall": rec.compute(predictions=preds, references=labels, average='micro'),
        "f1": f1.compute(predictions=preds, references=labels, average='micro')
    }
